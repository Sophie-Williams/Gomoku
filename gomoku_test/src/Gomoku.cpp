...
{
  mPlayers.push_back(std::make_unique<HumanPlayer>(PlayerSymbol::BLACK_CASE));

  std::cout << "player1 symbol: " << (char)PlayerSymbol::BLACK_CASE << std::endl;

  mPlayers.push_back(std::make_unique<gmk::AIPlayer>(PlayerSymbol::WHITE_CASE));
//  mPlayers.push_back(std::make_unique<HumanPlayer>(PlayerSymbol::WHITE_CASE));
